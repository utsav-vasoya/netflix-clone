...
export const getUserLikedMovies = createAsyncThunk(
  ...
  async (email) => {
    ...
    const {data:{movies}} = await axios.get(`http://localhost:7600/api/user/liked/${email}`)
    return movies
...
export const removeFromLikedMovies = createAsyncThunk(
  "netflix/removeMovie",
  async ({email, movieId }) => {
    const {data:{movies}} = await axios.put(`http://localhost:7600/api/user/delete`,{email,movieId})
    return movies
  }
);
const NetflixSclice = createSlice({
  name: "Netflix",
  initialState,
